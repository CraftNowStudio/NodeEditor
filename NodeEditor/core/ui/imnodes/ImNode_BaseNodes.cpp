...
{
	outAttr = new OutAttr(value, "output", varType);
	push_outputAttr(outAttr);
};
OutNode::~OutNode() { delete outAttr; };
void OutNode::forward(){};
...
{
	in1 = new InAttr(value, "a ", varType);
	in2 = new InAttr(value, "b ", varType);
	out1 = new OutAttr(value, "c ", varType);
	push_inputAttr(in1);
	push_inputAttr(in2);
...
template <class valueType>
void AddNode<valueType>::forward() {
	valueType res = 0;
	// render attr
	for (auto it = inputAttrList.begin(); it < inputAttrList.end(); it++) {
...
template <class valueType>
void MulNode<valueType>::forward() {
	valueType res = 1;
	// render attr
	for (auto it = inputAttrList.begin(); it < inputAttrList.end(); it++) {
		res *= std::any_cast<valueType>(*((*it)->get()));
