...
#include "drivers/OpenGL/opengl3_driver.h"
...
namespace NodeEditor {
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		{
			NE_PROFILE_SCOPE("glfwInit");
			int success = glfwInit();
			NE_CORE_ASSERT(success, "GLFW初始化失败!");
			glfwSetErrorCallback(GLFWErrorCallback);
		}

		{
			NE_PROFILE_SCOPE("glfwCreateWindow");
		...
		//根据平台不同抽象出渲染API
		m_Context = new OpenGLDriver(m_Window);
		m_Context->Init();
