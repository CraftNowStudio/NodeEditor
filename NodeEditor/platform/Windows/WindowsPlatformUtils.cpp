...
bool FileDialogs::createDirectory(const std::string &path) {
	...
	try {
		if (!fs::exists(directoryPath)) {
			...
			NE_CORE_TRACE("创建文件夹：{0}", directoryPath.string());
		} else {
			NE_CORE_TRACE("路径：{0}, 已存在", directoryPath.string());
		}
		return true;
	} catch (const std::exception &e) {
		NE_CORE_ERROR("创建文件夹失败: {0}", e.what());
		return false;
	}
}
...
void FileDialogs::copyFile(const std::filesystem::path &source, const std::filesystem::path &destination) {
	try {
		...
		NE_CORE_TRACE("文件复制成功！");
	} catch (const std::filesystem::filesystem_error &e) {
		NE_CORE_ERROR("文件复制失败！..");
	}
}
} //namespace NodeEditor
